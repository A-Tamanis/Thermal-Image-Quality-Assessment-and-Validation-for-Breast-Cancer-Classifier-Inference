...
MIN_CONF_STRONG = 0.5   # right_breast, armpit
...
RB_X_MIN = 0.35   # not too far left
RB_X_MAX = 0.80   # not hugging right border
RB_Y_MIN = 0.40   # Y-axis noralized from the top of the image
RB_Y_MAX = 0.85   # not in the lower 15% of image height

# Armpit must be clearly above right breast (margin in normalized units)
...
# Nipple Thresholds
MAX_NIP_DIST_WIDTH_RATIO = 0.45  #nipple distance to image width ratio
MIN_NIP_DIST_WIDTH_RATIO = 0.25
MAX_NIP_ANG = 20.0
MIN_NIP_ANG = -10.0
...
def _select_best_boxes(
    ...
    for det in detections:
        ...
        # confidence threshold assigning
        #min_conf = MIN_CONF_STRONG if name in ("right_breast", "armpit") else MIN_CONF_WEAK
        min_conf = MIN_CONF_STRONG if name in ("right_breast") else MIN_CONF_WEAK
        if conf < min_conf:
            continue
...
def evaluate_pose(
    ...
    """
    Evaluate whether the patient is in a good ~45 degree pose based on YOLO detections
    ...
    # function to add a scoring component
    ...
    # Hard checks, fail if at least one rule fails
    hard_fail = False
    ...
    # Right breast size & position (hard and soft rules)

    # right breast area fraction
    ...
    # center position of right breast (x, y)
    ...
    # Armpit relation (hard and soft rules)
    ...
    # soft: armpit horizontal position – should be same or more to the right of breast
    if ap.cx_n >= rb.cx_n:
        score += 1.0
        reasons.append(
            "OK: armpit horizontally at or to the right of right_breast"
        )
    else:
        reasons.append(
    ...
    # Border and cropping checks (soft)
    ...
    # Uncomment to add border violations to scoring system
    #add_component(rb_border_s, weight=1.0, desc="right_breast border margin score")
    #add_component(ap_border_s, weight=1.0, desc="armpit border margin score")

    if rb_border_s <= 0.0:
        reasons.append("HARD FAIL: right_breast too close to image borders")

        if ap_border_s <= 0.0:
            reasons.append("HARD FAIL: armpit too close to image borders")
    ...
    reasons.append("OK: left_breast and armpit inside image borders")
    ...
    if lb is not None:
        ...
        # left breast should be left of right_breast
        if lb.cx_n < rb.cx_n:
            score += 0.5
            reasons.append("OK: left_breast is left of right_breast")
        else:
        ...
        if lb.area_frac <= rb.area_frac:
            score += 0.5
            reasons.append("OK: left_breast area <= right_breast area")
        else:
    ...
    # nipple inside own breast (if real)
    if rn is not None:
        max_score += 0.5
    ...
    # nip-to-nip line geometry (only if we have both breasts)
    if lb is not None:
        dx = (rn_x - ln_x)
        dy = (rn_y - ln_y)
        ...
        dist_nx_wx = abs(dx)/img_w

        fail = False

        if angle > MAX_NIP_ANG:
            reasons.append(f"HARD FAIL: nipple angle too high, breasts not level (angle={angle:.1f}° > {MAX_NIP_ANG}) ")
            fail = True

        
        if angle < MIN_NIP_ANG:
            reasons.append(f"HARD FAIL: nipple angle too low, breasts not level (angle={angle:.1f}° > {MIN_NIP_ANG}) ")
            fail = True

        
        if dist_nx_wx > MAX_NIP_DIST_WIDTH_RATIO:
            reasons.append(f"HARD FAIL: nipple horizontal distance to image width too high, adjust capture angle (distance ratio={dist_nx_wx:.2f} > {MAX_NIP_DIST_WIDTH_RATIO}) ")
            fail = True


        if dist_nx_wx < MIN_NIP_DIST_WIDTH_RATIO:
            reasons.append(f"HARD FAIL: nipple horizontal distance to image width too small, adjust capture angle (distance ratio={dist_nx_wx:.2f} > {MIN_NIP_DIST_WIDTH_RATIO}) ")
            fail = True
        ...
        if fail:
            ...
            reasons.append(f"OK: nipple angle normal (angle={angle:.1f}°) ")
            reasons.append(f"OK: nipple distance to image width (ratio={dist_nx_wx:.2f}) ")


        # Uncomment to add nipple angle to scoring system instead of hard fail
        # Expect nip line to be more horizontal than vertical, horiz_frac near 1
        # horiz_score = max(0.0, min(1.0, (horiz_frac - 0.5) / 0.5))  
