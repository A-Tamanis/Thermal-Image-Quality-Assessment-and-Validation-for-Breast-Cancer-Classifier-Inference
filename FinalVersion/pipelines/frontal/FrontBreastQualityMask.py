...
def make_breast_mask_from_body(
    body_mask,
    top_crop_frac=0.20,    # remove 20% of torso height from the top
    bottom_crop_frac=0.20,  # remove 20% of torso height from the bottom
    erode_size=40
):
    """
    ...
    #cv2.imshow("eroded body mask", (body_mask*255).astype(np.uint8))
...
def evaluate_breast_quality_on_mask(
    ...
    # thresholds
    max_symmetry_ratio=1.5,     # max area_left/area_right ratio
    ...
    max_cold_frac=0.10,         # max allowed cold-blob fraction of breast band area
    ...
    # left-right symmetry on band_mask
    mid_x = W_band // 2
    left_mask = band_mask[:, :mid_x]
    right_mask = band_mask[:, mid_x:]

    area_left = int(left_mask.sum())
    area_right = int(right_mask.sum())
    metrics["area_left"] = area_left
    metrics["area_right"] = area_right

    if min(area_left, area_right) > 0:
        symmetry_ratio = max(area_left, area_right) / float(min(area_left, area_right))
    else:
    ...
    rules = {
        ...
        "symmetry_ok": symmetry_ratio <= max_symmetry_ratio,
...
def check_background_quality(
    ...
    body_margin_size=15,     # margin around body excluded from background
    threshold=25.0           # Laplacian variance threshold
):
    """
    Returns:
    ...
    # Background block statistics check
    block_size = 64  
    mean_thresh = 40.0
    var_thresh  = 90.0
...
def run_check(image_path: str) -> Dict[str, Any]:
    """
    Pipeline wrapper for 0 degree background check
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="0 metrics / breast-band quality check")
